/// # Flutter TypeAhead
/// A TypeAhead widgets for Flutter, where you can show suggestions to
/// users as they type
///
/// ## Features
...
/// suggestions.
///
/// The `itemBuilder` is called to build a widgets for each suggestion.
...
/// callback here. The TypeAhead widgets takes care of that.
///
...
/// `animationController` as parameters, and should return a widgets that uses
...
class TypeAheadFormField<T> extends FormField<String> {
  ...
  /// that the TypeAhead widgets displays
  final TextFieldConfiguration textFieldConfiguration;

  /// callback for pagination
  final Function? onInfiniteScroll;

  final GlobalKey<TypeAheadFieldState>? fieldKey;
    ...
    this.onInfiniteScroll,
        ...
        super(
            ...
            builder: (FormFieldState<String> field) {
              final TypeAheadFormFieldState state = field as TypeAheadFormFieldState<dynamic>;
...
class TypeAheadFormFieldState<T> extends FormFieldState<String> {
  ...
  void _handleControllerChanged() {
    ...
    //
    // In the case where a controller has been passed in to this widgets, we
...
class TypeAheadField<T> extends StatefulWidget {
  ...
  ///
  /// This callback must not be null. It is be called by the TypeAhead widgets
  ...
  ///
  /// This callback must not be null. It is called by the TypeAhead widgets and
  ...
  ///
  /// This callback must not be null. It is called by the TypeAhead widgets for
  /// each suggestion, and expected to build a widgets to display this
  ...
  ///
  /// It is expected to return a widgets to display while waiting.
  /// For example:
  /// ```dart
  ...
  ///
  /// It is expected to return a widgets to display when no suggestions are
  /// available.
  /// For example:
  /// ```dart
  /// (BuildContext context) {
  ...
  ///
  /// It is called with the error object, and expected to return a widgets to
...
class TypeAheadFieldState<T> extends State<TypeAheadField<T>> with WidgetsBindingObserver {
  final GlobalKey<SuggestionsListState> suggestionsKey = GlobalKey();
  ...
  // Will have a value if the typeahead is inside a scrollable widgets
  ...
  void _initOverlayEntry() {
    this._suggestionsBox!._overlayEntry = OverlayEntry(builder: (context) {
      final suggestionsList = _SuggestionsList<T>(
        ...
        onInfiniteScroll: widget.onInfiniteScroll,
        key: suggestionsKey,
      );
...
class SuggestionsListState<T> extends State<_SuggestionsList<T>> with SingleTickerProviderStateMixin {
  ...
  Widget createSuggestionsWidget() {
    Widget child = NotificationListener<ScrollNotification>(
      onNotification: (notification) {
        if (widget.onInfiniteScroll != null && notification.metrics.pixels == notification.metrics.maxScrollExtent) {
...
class TextFieldConfiguration {
  ...
  ///
  /// If null, this widgets will create its own [TextEditingController](https://docs.flutter.io/flutter/widgets/TextEditingController-class.html).
  /// A typical use case for this field in the TypeAhead widgets is to set the
  /// text of the widgets when a suggestion is selected. For example:
  ///
  /// ```dart
  ...
  /// Controls whether this widgets has keyboard focus.
  ///
...
class _SuggestionsBox {
  ...
  void resize() {
    // check to see if widgets is still mounted
    // user may have closed the widgets with the keyboard still open
    if (widgetMounted) {
